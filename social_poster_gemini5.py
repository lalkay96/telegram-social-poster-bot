...
async def post_to_twitter(caption: str, image_path: str = None):
    ...
    try:
        ...
        if twitter_v2_client is None or twitter_v1_api is None:
            # OAuth 2.0 User Handler for both v1.1 (media upload) and v2 (tweet posting)
            ...
            twitter_v2_client = tweepy.Client(
                ...
                access_token=TWITTER_ACCESS_TOKEN,
                access_token_secret=TWITTER_REFRESH_TOKEN # tweepy uses this for refresh token in OAuth2UserHandler
            )
            logger.info("🐦 Initialized Tweepy v2 Client.")

            # Initialize v1.1 API for media upload
            twitter_v1_api = tweepy.API(oauth2_user_handler)
