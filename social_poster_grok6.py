    ...
    try:
        async with httpx.AsyncClient() as client:
            if image_path and os.path.exists(image_path):
                ...
                logger.info("☁️ Uploading image to Cloudinary: %s", image_path)
                upload_result = await asyncio.to_thread(
                    cloudinary.uploader.upload, image_path, resource_type="image"
                )
                cloudinary_url = upload_result['secure_url']
                logger.info("✅ Image uploaded to Cloudinary. URL: %s", cloudinary_url)

                # Upload to Instagram Feed
                logger.info("📸 Uploading to Instagram Feed using Cloudinary URL: %s", cloudinary_url)
                response = await client.post(
                ...
                # Upload to Stories (optional, may require additional permissions)
