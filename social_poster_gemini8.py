...
TWITTER_ACCESS_TOKEN = os.getenv("TWITTER_ACCESS_TOKEN") # OAuth 2.0 Access Token
TWITTER_REFRESH_TOKEN = os.getenv("TWITTER_REFRESH_TOKEN") # OAuth 2.0 Refresh Token

# Twitter API v1.1 OAuth 1.0a User Context credentials (for media upload)
TWITTER_API_KEY_V1 = os.getenv("TWITTER_API_KEY_V1") # Consumer Key for OAuth 1.0a
TWITTER_API_SECRET_V1 = os.getenv("TWITTER_API_SECRET_V1") # Consumer Secret for OAuth 1.0a
TWITTER_ACCESS_TOKEN_V1 = os.getenv("TWITTER_ACCESS_TOKEN_V1") # Access Token for OAuth 1.0a
TWITTER_ACCESS_TOKEN_SECRET_V1 = os.getenv("TWITTER_ACCESS_TOKEN_SECRET_V1") # Access Token Secret for OAuth 1.0a
...
async def post_to_instagram_story(image_url: str, caption: str = None): # Added caption parameter
    """
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Prepare data payload for story
            story_data = {
                "image_url": image_url,
                "media_type": "STORIES", # This is the key difference for stories
                "access_token": IG_ACCESS_TOKEN
            }
            if caption: # Add caption if provided
                story_data["caption"] = caption
                logger.info("📸 Adding caption to IG Story: '%s'", caption)
...
async def post_to_twitter(caption: str, image_path: str = None):
    ...
    logger.info("Twitter Env Vars Loaded: CLIENT_ID=%s, CLIENT_SECRET=%s, ACCESS_TOKEN=%s, REFRESH_TOKEN=%s, API_KEY_V1=%s, API_SECRET_V1=%s, ACCESS_TOKEN_V1=%s, ACCESS_TOKEN_SECRET_V1=%s",
                ...
                TWITTER_API_KEY_V1,
                "*" * len(TWITTER_API_SECRET_V1) if TWITTER_API_SECRET_V1 else "None",
                "*" * len(TWITTER_ACCESS_TOKEN_V1) if TWITTER_ACCESS_TOKEN_V1 else "None",
                "*" * len(TWITTER_ACCESS_TOKEN_SECRET_V1) if TWITTER_ACCESS_TOKEN_SECRET_V1 else "None")
    ...
    try:
        ...
        if twitter_v2_client is None or twitter_v1_api is None:
            # Initialize v2 Client for tweet posting - this needs all credentials for user_auth=True
            twitter_v2_client = tweepy.Client(
                bearer_token=TWITTER_ACCESS_TOKEN, # OAuth 2.0 Access Token
                consumer_key=TWITTER_CLIENT_ID, # Consumer Key for OAuth 1.0a context
                consumer_secret=TWITTER_CLIENT_SECRET, # Consumer Secret for OAuth 1.0a context
                access_token=TWITTER_ACCESS_TOKEN, # OAuth 2.0 Access Token
                access_token_secret=TWITTER_REFRESH_TOKEN # OAuth 2.0 Refresh Token
            )
            logger.info("🐦 Initialized Tweepy v2 Client with full credentials.")

            # Initialize v1.1 API for media upload using OAuth1UserHandler
            # This uses the dedicated OAuth 1.0a tokens
        ...
        if e.response and e.response.status_code == 401 and "Unauthorized" in str(e.response.text):
            ...
            try:
                ...
                twitter_v2_client = tweepy.Client(
                    bearer_token=new_tokens['access_token'], # Update with new OAuth 2.0 Access Token
                    consumer_key=TWITTER_CLIENT_ID,
                    consumer_secret=TWITTER_CLIENT_SECRET,
                    access_token=new_tokens['access_token'],
                    access_token_secret=new_tokens['refresh_token'] # Update with new OAuth 2.0 Refresh Token
                )
                # Also re-initialize v1.1 API with the new tokens (using the new OAuth 2.0 access token for v1.1 handler)
                auth_v1 = tweepy.OAuth1UserHandler(
                    TWITTER_API_KEY_V1, # Use V1 API Key
                    TWITTER_API_SECRET_V1, # Use V1 API Secret
                    new_tokens['access_token'], # Use the new OAuth 2.0 Access Token for v1.1 handler
                    new_tokens['refresh_token'] # Use the new OAuth 2.0 Refresh Token for v1.1 handler
                )
                twitter_v1_api = tweepy.API(auth_v1)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        posting_tasks.append(post_to_instagram_story(image_url, caption)) # Pass caption here
