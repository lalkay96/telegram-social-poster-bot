...
async def post_to_twitter(caption: str):
    ...
    try:
        # Initialize tweepy client if not already done
        if twitter_client is None:
            # For OAuth 2.0 User Context, the Client is primarily initialized with the bearer_token (access token).
            # client_id and client_secret are used by the refresh_token method.
            twitter_client = tweepy.Client(
                bearer_token=TWITTER_ACCESS_TOKEN,
                # consumer_key=TWITTER_CLIENT_ID, # These are for OAuth 1.0a, not directly for OAuth 2.0 Client
                # consumer_secret=TWITTER_CLIENT_SECRET, # These are for OAuth 1.0a, not directly for OAuth 2.0 Client
                # access_token=TWITTER_ACCESS_TOKEN, # This is for OAuth 1.0a
                # access_token_secret=TWITTER_REFRESH_TOKEN # This is for OAuth 1.0a
            )
            logger.info("🐦 Initialized Tweepy client with provided access token.")
        ...
        # Note: Twitter API v2 access tokens expire in 2 hours.
        if e.response and e.response.status_code == 401 and "Unauthorized" in str(e.response.text):
            ...
            try:
                ...
                # This requires TWITTER_CLIENT_ID and TWITTER_CLIENT_SECRET (if confidential)
